...
async def oauth2callback_airtable(request: Request):
    ...
    # gets the metadata of state_data from authorize_airtable using encode/decode get from the api
    original_state = state_data.get('state')
    user_id = state_data.get('user_id')
    org_id = state_data.get('org_id')

    #get state and code from redis
    ...
    if not saved_state or original_state != json.loads(saved_state).get('state'):   # if the sent state doesnt match receiving state
        raise HTTPException(status_code=400, detail='State does not match.')        # raise error:400, state not match
